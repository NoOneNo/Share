...
/**
 * 每当用户可以通过一个垂直的滑动手势刷新一个view的内容时可以使用SwipeRefreshLayout。
 * 实例化这个view(SwipeRefreshLayout)的activity都应该实现OnRefreshListener
 * 方法，用于通知滑动刷新动作已经完成；SwipeRefreshLayout将会通知listener每当刷新动作
 * 再一次被触发。这个listener用于正确决定什么时候开始内容的刷新。如果这个listener认为不应
 * 该是一个刷新，必须调用setRefreshing(false)方法取消所有可见的刷新迹象。如果一个activity
 * 只是希望展示加载进度动画，应该调用setRefreshing(true)。要使滑动手势和加载进度动画不可用，
 * 可以调用这个view的setEnabled(false)。
 * <p>
 * 这个layout应该作为用于当滑动手势结束时刷新内容的view的父节点，并且只支持一个子节点（但是子
 * 节点可以包括其他子节点）。这个view（显示刷新内容的view）也将会被作为滑动手势的目标被迫匹配
 * 这个layout提供的宽度和高度。SwipeRefreshLayout不提供辅助功能事件，取而代之的是，不管
 * 滑动手势在哪里被触发，应该提供一个menu item用于触发内容的刷新。
 * <p>
 ...
 * </p>
 */
public class SwipeRefreshCustomLayout extends ViewGroup {
    // Maps to ProgressBar.Large style
    public static final int LARGE = MaterialProgressDrawable.LARGE;
    // Maps to ProgressBar default style
    public static final int DEFAULT = MaterialProgressDrawable.DEFAULT;

    private static final String LOG_TAG = SwipeRefreshCustomLayout.class.getSimpleName();

    private static final float MAX_ALPHA = 1.0f;
    
    private static final float STARTING_PROGRESS_ALPHA = .3f;
...
//    private static final int DEFAULT_CIRCLE_TARGET = 100;
    ...
    private Animation.AnimationListener mRefreshListener = new Animation.AnimationListener() {
        ...
        public void onAnimationEnd(Animation animation) {
            if (mRefreshing) {
            	...
            	mLoadingView.setAlpha(MAX_ALPHA);
            	animateStartLoadingRotate();
                if (mNotify) {
                    if (mListener != null) {
                        mListener.onRefresh();
                    }
    ...
    private void setColorViewAlpha(float targetAlpha) {
    	mLoadingView.setAlpha(targetAlpha);
...
//        mCircleView.setImageDrawable(mProgress);
     ...
     * @param context
     */
    public SwipeRefreshCustomLayout(Context context) {
        this(context, null);
    }
     ...
     * @param attrs
     */
    public SwipeRefreshCustomLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    	...
    	final TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.SwipeRefreshLayout);
    	Drawable src = ta.getDrawable(R.styleable.SwipeRefreshLayout_src);
    	ta.recycle();
    	mSrc = src;
        if(src != null){
        	Log.d(LOG_TAG, "Receive src!");
        }else{
        	Log.d(LOG_TAG, "Don't receive src!");
        }
    ...
    private void createProgressView() {
        ...
        ViewGroup.LayoutParams lp = new LayoutParams(mCircleWidth, mCircleWidth);
        mLoadingView.setLayoutParams(lp);
        mLoadingView.setBackground(mSrc);
        mLoadingView.setVisibility(View.GONE);
    ...
    private void setAnimationProgress(float progress) {
        if (isAlphaUsedForScale()) {
            setColorViewAlpha(progress * MAX_ALPHA);
        } else {
    ...
    private void startProgressAlphaStartAnimation() {
        mAlphaStartAnimation = startAlphaAnimation(mLoadingView.getAlpha(), STARTING_PROGRESS_ALPHA);
    }

    private void startProgressAlphaMaxAnimation() {
    	mAlphaMaxAnimation = startAlphaAnimation(mLoadingView.getAlpha(), MAX_ALPHA);
    }
    ...
    private Animation startAlphaAnimation(final float startingAlpha, final float endingAlpha) {
        ...
        Animation alpha = new Animation() {
            ...
            public void applyTransformation(float interpolatedTime, Transformation t) {
            	mLoadingView
                        .setAlpha(startingAlpha + ((endingAlpha - startingAlpha)
                                * interpolatedTime));
            	mLoadingView.invalidate();
            }
        };
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mIsBeingDragged) {
                    ...
                    if (overscrollTop < mTotalDragDistance) {
                        ...
                        if (mLoadingView.getAlpha() < MAX_ALPHA
    ...
    private void startScaleDownReturnToStartAnimation(int from,
        ...
        mFrom = from;
        if (isAlphaUsedForScale()) {
        	mStartingScale = mLoadingView.getAlpha();
        } else {
    ...
    private void setTargetOffsetTopAndBottom(int offset, boolean requiresUpdate) {
    	Log.i(LOG_TAG, "setTargetOffsetTopAndBottom invoke, offset : " + offset);
    	Log.i(LOG_TAG, "mLoadingView.getTop() : " + mLoadingView.getTop());
