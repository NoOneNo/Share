...
public class StatusNotifyFragmentPager extends FragmentPagerAdapter {

    public StatusNotifyFragmentPager(FragmentManager fm, Context context, List<StatusPresenter.StatusGroup> statusGroupGroups) {
        super(fm);
        mContext = context;
    ...
    SparseArray<StatusNotifyFragment> mFragments;
    ...
    @Override
    public StatusNotifyFragment getItem(int position) {
        StatusNotifyFragment fragment = mFragments.get(position);
        if (fragment == null) {
            fragment = StatusNotifyFragment.newInstance(mStatusGroupGroups.get(position));
